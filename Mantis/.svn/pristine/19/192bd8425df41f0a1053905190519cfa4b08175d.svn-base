...
class IssueNoteAddCommand extends Command {
	...
	function validate() {
		...
		if( isset($_SESSION['is_out_user']) ) {
		    # 判断是否评价过了
            if(!bug_has_pingjia_is_solved($this->data['query']['issue_id'])) {
                # 判断是否是评价
                $text = $this->data['payload']['text'];
                if(is_serialized($text)) {
                    $textArr = unserialize($text);
                    $is_pingjia = $textArr['is_pingjia'];
                }
            } else {
                print_error_page('一个问题只能评价一次');
            }
		...
		# 评价的话是可以插入的（但是评价只能一次）
		if( bug_is_readonly( $t_issue_id ) && !$is_pingjia) {
            print_error_page('问题已解决，无法进行反馈');
            throw new ClientException(
        ...
        # 这里附加条件：客户可添加注释（加急，当然仅限自己项目的问题）
		if(!isset($_SESSION) && !access_has_bug_level( config_get( 'add_bugnote_threshold' ), $t_issue_id, $this->reporterId ) ) {
