 ...
 * @return string
 */
function prepare_user_name( $p_user_id, $bug_id=0 ) {
	...
	if(!empty($bug_id)) {
	    // 判断问题是否工单问题
        $sql = "select category_id from {bug} where id = ".db_param();
        $t_result = db_query($sql,[$bug_id]);
        $arr = db_fetch_array($t_result);
        if(!empty($arr) && $arr['category_id'] == 15) {
            $sql = "select username,realname from {out_user} where id = ".db_param();
            $t_result = db_query($sql,[$p_user_id]);
            $arr = db_fetch_array($t_result);
            if(!empty($arr)) {
                return $arr['realname'];
            }
...
//    $sql = "select realname from {user} where id = ".db_param();
//	$t_result = db_query($sql,[$p_user_id]);
//	$arr = db_fetch_array($t_result);
//
//	// 找不到后台用户则考虑前台用户的情况
//	if(empty($arr)) {
//        $sql = "select realname from {out_user} where id = ".db_param();
//        $t_result = db_query($sql,[$p_user_id]);
//        $arr = db_fetch_array($t_result);
//        if(!empty($arr)) {
//            return $arr['realname'];
...
//    }
	...
	$t_username = string_display_line( $t_username ).'('.$arr['realname'].')';
