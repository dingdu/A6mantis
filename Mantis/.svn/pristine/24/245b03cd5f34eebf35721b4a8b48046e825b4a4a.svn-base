...
<?php }
	...
	for( $i=0; $i < $t_activities_count; $i++ ) {
		...
		$is_out_user = false;
		# 反序列化（如果可以序列化）
        if(is_serialized($t_activity['note']->note)){
            $t_activity['note']->note = unserialize($t_activity['note']->note);
            $is_out_user = $t_activity['note']->note['is_out_user'];
            $is_pingjia = $t_activity['note']->note['is_pingjia'];
            # 判断内容是评价还是评论
            if($is_pingjia) {
                if($t_activity['note']->note['isSolve']) {
                    $isSolve = '是';
                } else {
                    $isSolve = '否';
                }
                # 评价的情况
                $t_activity['note']->note = '【用户评价 】<br/>'.
                    '&nbsp;整体评价: '. $t_activity['note']->note['star'] .'星<br/>'.
                    '&nbsp;问题是否解决：'. $isSolve .'<br/>'.
                    '&nbsp;评论内容: '. $t_activity['note']->note['pingjia'];
            } else {
                # 评论的情况
                $t_activity['note']->note = $t_activity['note']->note['content'];
            }
        }
...
<tr class="bugnote visible-on-hover-toggle" id="c<?php echo $t_activity['id'] ?>">
		...
		<p class="no-margin">
			...
			print_user( $t_activity['user_id'], $f_bug_id );
			?>
		</p>
		...
		<p class="no-margin">
            ...
            if ($is_out_user) {
                # 判断是否是客户的注释
                echo '<span class="label label-sm label-default ' . $t_label . '">', 客户, '</span>';
            }
            else if( user_exists( $t_activity['user_id'] ) ) {
