...
class IssueAddCommand extends Command {
	...
	function validate() {
		...
		if( !isset($_SESSION['is_out_user']) && !access_has_project_level( config_get( 'report_bug_threshold' ), $t_project_id, $this->user_id ) ) {
			throw new ClientException(
		...
		$t_evaluate_time= isset( $t_issue['evaluate_time'] ) ? $t_issue['evaluate_time'] :0;

		// 预计开始时间和预计结束时间
		$t_expected_starttime = isset( $t_issue['expected_starttime'] ) ? $t_issue['expected_starttime'] : 'null';
		$t_expected_endtime = isset( $t_issue['expected_endtime'] ) ? $t_issue['expected_endtime'] : 'null';
        ...
        # 这里允许用户对应的bug_report指定项目负责人为问题的负责人
		if( $t_handler_id > 0 && !isset($_SESSION['is_out_user'])) {
		...
		$this->issue->evaluate_time = $t_evaluate_time;
		...
		$this->issue->expected_starttime = $t_expected_starttime;
		$this->issue->expected_endtime = $t_expected_endtime;
