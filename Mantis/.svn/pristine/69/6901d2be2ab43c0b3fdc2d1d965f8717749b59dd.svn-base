...
if( $f_master_bug_id > 0 ) {
	...
	if(isset($_SESSION['is_out_user'])){
        $cur_project = out_user_get_field(auth_get_current_user_id(), 'project_id');
        if($cur_project == 0) {
            print_error_page('您未有项目或未绑定项目，请联系服务人员将您规定项目');
        }
    } else {
        # 当前项目id
...
$t_expected_starttime = in_array( 'expected_starttime', $t_fields );
$t_expected_endtime = in_array( 'expected_endtime', $t_fields );
$t_show_tags =
...
<!--	<tr>-->
<!--		<td class="category">-->
<!--			<label>--><?php //echo lang_get( 'select_project_button' ) ?><!--</label>-->
<!--		</td>-->
<!--		<td>-->
...
<!--	</tr>-->
    ...
    if(!isset($_SESSION['is_out_user'])) {
        ...
        # 获取项目名称
        $project_name = project_get_field($project_id, 'name');
        ?>
        <!-- select -->
<!--        <tr>-->
<!--            <td class="category">-->
...
<!--                </select>-->
                ...
                <input type="text"  readonly="readonly"
                       name="project_id" value="<?php echo $project_name; ?>">
            </td>
        </tr>
        <?php
    }
...
<?php
	...
	if( $t_show_category ) {
?>
	<tr <?php if(isset($_SESSION['is_out_user'])) echo 'style="display:none"'; ?>>
		...
		<td width="85%">
			...
			<select <?php echo helper_get_tab_index() ?> id="category_id" name="category_id" class="autofocus input-sm" required>
				<?php
		...
		<tr <?php if(isset($_SESSION['is_out_user'])) echo 'style="display:none"'; ?>>
			<td class="category">
				<span class="required">*</span><label><?php echo lang_get( 'estimate_working_time' ) ?></label>
			</td>
			<td>
				<div class="input-group" style="width: 200px;float: left">
					<input type="number"  value="5" id="evaluate_time" name="evaluate_time"  min="0.1" step="0.1" required="required" class="form-control">
<!--					<span class="input-group-addon">--><?php //echo lang_get( 'working_hours_day' ) ?><!--</span>-->
    ...
    if(!isset($_SESSION['is_out_user'])) {
        # 预计开始时间
        if ($t_expected_starttime) {
            ?>

            <tr>
                <th class="category">
                    <label for="expected_starttime"><?php echo '预计开始时间'; ?></label>
                </th>
                <td>
                    <input id="expected_starttime" name="expected_starttime" type="text">
                </td>
            </tr>
            <?php
        }

        if ($t_expected_endtime) {
            ?>

            <tr>
                <th class="category">
                    <label for="expected_endtime"><?php echo '预计结束时间' ?></label>
                </th>
                <td>
                    <input id="expected_endtime" name="expected_endtime" type="text">
                </td>
            </tr>
            <!--导入datepickerjs-->
            ...
            <script>
                $('#expected_starttime').datetimepicker({
                    format: 'YYYY-MM-DD HH:mm'//显示格式
                });
                $('#expected_endtime').datetimepicker({
                    format: 'YYYY-MM-DD HH:mm'//显示格式
                });
            </script>
            <?php
        ...
        <?php if ($t_show_product_build) { ?>
            <tr>
                ...
                <td>
                    <input <?php echo helper_get_tab_index() ?> type="text" id="build" name="build" size="32"
                                                                maxlength="32"
                                                                value="<?php echo string_attribute($f_build) ?>"/>
                </td>
            </tr>
        <?php } ?>
...
<?php event_signal( 'EVENT_REPORT_BUG_FORM', array( $t_project_id ) ) ?>
    ...
    <?php if(isset($_SESSION['is_out_user'])){ ?>
        <!-- 客户才需要填的模块 -->
        ...
        <tr>
            <th class="category">
                <span class="required">*</span><label for="run_platform">运行环境</label>
            </th>

            <td>
                <select name="run_platform" id="run_platform"  required>
                    <option value="1">电脑PC</option>
                    <option value="2">安卓手机</option>
                    <option value="3">苹果手机</option>
                    <option value="4">平板电脑</option>
                </select>
                <!--具体的型号-->
                <input type="text" id="specific_platform" name="specific_platform" placeholder="请输入浏览器名称" required>
                <script>
                    $('#run_platform').change(function() {
                        // alert($('#run_platform').val());
                        var v = $('#run_platform').val();
                        var placeholder = '';
                        if(v == 1) {
                            placeholder = '请输入浏览器名称';
                        } else if(v == 2 || v == 3) {
                            placeholder = '请输入手机型号+浏览器名称';
                        } else {
                            placeholder = '请输入平板型号+浏览器名称';
                        }
                        $('#specific_platform').attr('placeholder', placeholder);
                    });
                </script>
            </td>
        </tr>
        ...
        <tr>
            <th class="category">
                <span class="required">*</span><label for="step">出现频率</label>
            </th>
            <td>
                <select name="reproducibility" id="kehu_reproducibility" required>
    ...
    <tr>
        <th class="category">
            <span class="required">*</span><label for="step">重现步骤</label>
        </th>

        <td>
            <textarea class="form-control" <?php echo helper_get_tab_index() ?> id="step" name="step" cols="80" rows="10" required></textarea>
        </td>
    </tr>
    <?php } ?>
...
<?php if( $t_show_steps_to_reproduce && !isset($_SESSION['is_out_user']) ) { ?>
		<tr>
			<th class="category">
...
if( $t_show_additional_info ) { ?>
    <tr>
        ...
        <td>
            <textarea class="form-control" <?php echo helper_get_tab_index() ?> id="additional_info"
                      name="additional_info" cols="80"
                      rows="10"><?php echo string_textarea($f_additional_info) ?></textarea>
        </td>
    </tr>
    <?php } ?>
    <?php if( $t_show_tags ) { ?>
...
<div class="widget-toolbox padding-8 clearfix">
	...
	<input <?php echo helper_get_tab_index() ?> type="submit" class="btn btn-primary btn-white btn-round"
            value="<?php if(!isset($_SESSION['is_out_user'])) echo lang_get( 'submit_report_button' ); else echo '提交工单'; ?>" />
</div>
</div>
</div>
</form>
...
<?php
    if(!isset($_SESSION['is_out_user'])) {
?>
var ue1 = UE.getEditor('additional_info');
document.getElementById('additional_info').className='edui-default';
var ue2 = UE.getEditor('steps_to_reproduce');
document.getElementById('steps_to_reproduce').className='edui-default';
<?php } else { ?>
var ue = UE.getEditor('step');
document.getElementById('step').className='edui-default';
<?php } ?>
</script>
<?php
layout_page_end();
