...
# 预计开始时间
echo '<th class="bug-status category">', '预计开始时间', '</th>';

echo '<td class="bug-resolution">';
echo '<input id="expected_starttime" name="expected_starttime" type="text" value="
';
if(!empty($t_bug->expected_starttime)) {
    echo date('Y-m-d H:i:s', $t_bug->expected_starttime);
}
echo '"> </td>';

# 预计结束时间
echo '<th class="bug-resolution category">', '预计结束时间', '</th>';
echo '<td class="bug-resolution">';
echo '<input id="expected_endtime" name="expected_endtime" type="text" value="
';
if(!empty($t_bug->expected_endtime)) {
    echo date('Y-m-d H:i:s', $t_bug->expected_endtime);
}
echo '"> </td>';
...
var ue1 = UE.getEditor('steps_to_reproduce');
document.getElementById('steps_to_reproduce').className='edui-default';
var ue2 = UE.getEditor('additional_information');
document.getElementById('additional_information').className='edui-default';
var ue3 = UE.getEditor('bugnote_text');
