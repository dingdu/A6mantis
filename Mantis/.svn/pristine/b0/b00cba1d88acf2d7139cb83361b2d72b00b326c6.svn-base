 ...
 * @return void
 */
function work_log_event_direct( $p_bug_id, $p_field_name, $p_old_value, $p_new_value, $p_user_id = null, $p_type = 0 ) {
	...
	if( $p_new_value != $p_old_value ) {
		...
		if( is_null( $p_new_value ) ) {
			...
			$c_new_value = mb_strimwidth( $p_new_value, 0, DB_FIELD_SIZE_work_VALUE, '...' );
		}

		db_param_push();
		$t_query = 'INSERT INTO {bug_work}
 ...
 * @return void
 */
function work_log_event( $p_bug_id, $p_field_name, $p_old_value ) {
	work_log_event_direct( $p_bug_id, $p_field_name, $p_old_value, bug_get_field( $p_bug_id, $p_field_name ) );
}
 ...
 * @return void
 */
function work_log_event_special( $p_bug_id, $p_type, $p_old_value = '', $p_new_value = '' ) {
	...
	if( is_null( $p_new_value ) ) {
		...
		$p_new_value = mb_strimwidth( $p_new_value, 0, DB_FIELD_SIZE_work_VALUE, '...' );
	}
...
/**
 * Retrieves the work events for the specified bug id and returns it in an array
 ...
 * @return array
 */
function work_get_events_array( $p_bug_id, $p_user_id = null ) {
	$t_normal_date_format = config_get( 'normal_date_format' );
	$t_query = 'SELECT * FROM   {user_work_log} as uwl left join {user} as u on u.id = uwl.user_id  WHERE uwl.task_bug_id=' . db_param();
...
//	$t_raw_work = work_get_raw_events_array( $p_bug_id, $p_user_id );
//	$t_work = array();
//
//	foreach( $t_raw_work as $k => $t_item ) {
//		extract( $t_item, EXTR_PREFIX_ALL, 'v' );
//		$t_work[$k] = work_localize_item( $v_field, $v_type, $v_old_value, $v_new_value );
//		$t_work[$k]['date'] = date( $t_normal_date_format, $v_date );
//		$t_work[$k]['userid'] = $v_userid;
//		$t_work[$k]['username'] = $v_username;
//	}
 ...
 */
function work_count_user_recent_events( $p_duration_in_seconds, $p_user_id = null ) {
	$t_user_id = ( ( null === $p_user_id ) ? auth_get_current_user_id() : $p_user_id );

	$t_params = array( db_now() - $p_duration_in_seconds, $t_user_id );

	db_param_push();
	$t_query = 'SELECT count(*) as event_count FROM {bug_work} WHERE date_modified > ' . db_param() .
...
/**
 * Creates and executes a query for the work rows, returning a database result object.
 ...
 * - "filter" => filter array       A filter array to limit work to bugs matched by this filter.
 * - "order" => Sort order          'ASC' or 'DESC' for result order.
 *
 * Any option can be omitted.
 *
 * @param array $p_query_options	Array of query options
 * @return database result to pass into work_get_event_from_row().
 */
function work_query_result( array $p_query_options ) {
	# check query order by
	if( isset( $p_query_options['order'] ) ) {
		$t_work_order = $p_query_options['order'];
	} else {
		$t_work_order = config_get( 'work_order' );
	}
	...
	if( isset( $p_query_options['filter'] ) ) {
		...
		$t_where[] = '{bug_work}.bug_id IN ' . $t_query->param( $t_subquery );
	}

	# Start time
	if( isset( $p_query_options['start_time'] ) ) {
		$t_where[] = '{bug_work}.date_modified >= ' . $t_query->param( (int)$p_query_options['start_time'] );
	}

	# End time
	if( isset( $p_query_options['end_time'] ) ) {
		$t_where[] = '{bug_work}.date_modified < ' . $t_query->param( (int)$p_query_options['end_time'] );
	}
	...
	if( isset( $p_query_options['bug_id'] ) ) {
		...
		$t_where[] = $t_query->sql_in( '{bug_work}.bug_id', $c_ids );
	}
	...
	if( isset( $p_query_options['user_id'] ) ) {
		...
		$t_where[] = $t_query->sql_in( '{bug_work}.user_id', $c_ids );
	}

	$t_query->append_sql( 'SELECT * FROM {bug_work}' );
	if ( count( $t_where ) > 0 ) {
		$t_query->append_sql( ' WHERE ' . implode( ' AND ', $t_where ) );
	}

	# Order work lines by date. Use the storing sequence as 2nd order field for lines with the same date.
	$t_query->append_sql( ' ORDER BY {bug_work}.date_modified ' . $t_work_order . ', {bug_work}.id ' . $t_work_order );
	$t_result = $t_query->execute();
	return $t_result;
}

/**
 * Creates and executes a query for the work rows related to bugs matched by the provided filter
 ...
 * @param  string  $p_work_order  The sort order.
 * @return database result to pass into work_get_event_from_row().
 * @deprecated		Use work_query_result() instead
 */
function work_get_range_result_filter( $p_filter, $p_start_time = null, $p_end_time = null, $p_work_order = null ) {
	error_parameters( __FUNCTION__ . '()', 'work_query_result()' );
	trigger_error( ERROR_DEPRECATED_SUPERSEDED, DEPRECATED );

	$t_query_options = array();
	if ( $p_work_order !== null ) {
		$t_query_options['order'] = $p_work_order;
	}
	...
	return work_query_result( $t_query_options );
}

/**
 * Creates and executes a query for the work rows matching the specified criteria.
 * @param  integer $p_bug_id         The bug id or null for matching any bug.
 * @param  integer $p_start_time     The start time to filter by, or null for all.
 * @param  integer $p_end_time       The end time to filter by, or null for all.
 * @param  string  $p_work_order  The sort order.
 * @return IteratorAggregate|boolean database result to pass into work_get_event_from_row().
 * @deprecated		Use work_query_result() instead
 */
function work_get_range_result( $p_bug_id = null, $p_start_time = null, $p_end_time = null, $p_work_order = null ) {
	error_parameters( __FUNCTION__ . '()', 'work_query_result()' );
	trigger_error( ERROR_DEPRECATED_SUPERSEDED, DEPRECATED );
...
/**
 * Gets the next accessible work event for current user and specified db result.
 ...
 *                                          false otherwise.
 * @return array containing the work event or false if no more matches.
 */
function work_get_event_from_row( $p_result, $p_user_id = null, $p_check_access_to_issue = true ) {
	...
	while ( $t_row = db_fetch_array( $p_result ) ) {
		...
		if( $v_type == BUG_ADD_RELATIONSHIP || $v_type == BUG_DEL_RELATIONSHIP || $v_type == BUG_REPLACE_RELATIONSHIP ) {
			...
			# accessible to user or not.  This also simplifies client code that is accessing the work log.
...
/**
 * Retrieves the raw work events for the specified bug id and returns it in an array
 ...
 * @return array
 */
function work_get_raw_events_array( $p_bug_id, $p_user_id = null, $p_start_time = null, $p_end_time = null ) {
	$t_user_id = (( null === $p_user_id ) ? auth_get_current_user_id() : $p_user_id );
	...
	$t_result = work_query_result( $t_query_options );

	$t_raw_work = array();

	$j = 0;
	while( true ) {
		$t_event = work_get_event_from_row( $t_result, $t_user_id, /* check access */ true );
		if ( $t_event === false ) {
			break;
		}

		$t_raw_work[$j] = $t_event;
		$j++;
	}
 ...
 */
function work_localize_field_name( $p_field_name ) {
	switch( $p_field_name ) {
		case 'category':
 ...
 */
function work_get_type_name( $p_type ) {
	$t_type = (int)$p_type;
...
/**
 * Localizes one raw work item specified by set the next parameters: $p_field_name, $p_type, $p_old_value, $p_new_value
 ...
 * @param integer $p_type       The type of the work entry.
 * @param string  $p_old_value  The old value of the field.
 * @param string  $p_new_value  The new value of the field.
 * @param boolean $p_linkify    Whether to return a string containing hyperlinks.
 * @return array
 */
function work_localize_item( $p_field_name, $p_type, $p_old_value, $p_new_value, $p_linkify = true ) {
	$t_note = '';
	$t_change = '';
	$t_raw = true;

	if( PLUGIN_work == $p_type ) {
	...
	$t_field_localized = work_localize_field_name( $p_field_name );
	switch( $p_field_name ) {
		case 'status':
		...
		if( NORMAL_TYPE != $p_type ) {
			switch( $p_type ) {
				...
				case BUG_ADD_RELATIONSHIP:
					...
					$t_change = relationship_get_description_for_work( $p_old_value ) . ' ' . bug_format_id( $p_new_value );
					break;
				case BUG_REPLACE_RELATIONSHIP:
				...
				case BUG_CREATED_FROM:
					...
					break;
				case TAG_ATTACHED:
					$t_note = lang_get( 'tag_work_attached' ) . ': ' . $p_old_value;
					break;
				case TAG_DETACHED:
					$t_note = lang_get( 'tag_work_detached' ) . ': ' . $p_old_value;
					break;
				case TAG_RENAMED:
					$t_note = lang_get( 'tag_work_renamed' );
					$t_change = $p_old_value . ' => ' . $p_new_value;
					break;
				case BUG_REVISION_DROPPED:
					$t_note = lang_get( 'bug_revision_dropped_work' ) . ': ' . bug_revision_get_type_name( $p_new_value ) . ': ' . $p_old_value;
					break;
				case BUGNOTE_REVISION_DROPPED:
					$t_note = lang_get( 'bugnote_revision_dropped_work' ) . ': ' . $p_new_value . ': ' . $p_old_value;
					break;
			}
	}
...
/**
 * delete all work associated with a bug
 * @param integer $p_bug_id A valid bug identifier.
 * @return void
 */
function work_delete( $p_bug_id ) {
	db_param_push();
	$t_query = 'DELETE FROM {bug_work} WHERE bug_id=' . db_param();
