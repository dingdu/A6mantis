...
/**
 * Notes:客户端登录cookie的校验
 * User: dingduming
 * Date: 2018\7\21 0021
 * Time: 14:45
 * @param string $p_return_page
 */
function out_auth_ensure_user_authenticated( $p_return_page = '' ) {
    # 客户已经登陆
    if( auth_is_user_authenticated() ) {
        # check for access enabled
        #  This also makes sure the cookie is valid
        ...
        # 没有登录则跳转到客户登陆页面
        print_header_redirect( 'feedback.php' );
    }
}
...
function auth_attempt_login( $p_username, $p_password, $p_perm_login = false ) {
	...
	if( !user_is_anonymous( $t_user_id ) ) {
        ...
        # 标记是后台用户登录（以便前账号是out_user的情况下查询的是user表）
		if( !auth_does_password_match( $t_user_id, $p_password,1 ) ) {
 ...
 * @access public
 */
function out_auth_attempt_login( $p_username, $p_password, $p_perm_login = false ) {
//    $t_user_id = auth_get_user_id_from_login_name( $p_username );
//
//    if( $t_user_id === false ) {
//        $t_user_id = auth_auto_create_user( $p_username, $p_password );
...
//    }
    ...
    return check_login_out_user($p_username, $p_password,$p_perm_login);

//    return out_auth_login_user( $t_user_id, $p_perm_login );
}
 ...
 * @access public
 */
function auth_does_password_match( $p_user_id, $p_test_password , $is_user_login = 0) {
    ...
    if(isset($_SESSION['is_out_user']) && $is_user_login != 1) {
        $t_password = out_user_get_field( $p_user_id, 'password' );
    } else {
 ...
 * @return void
 */
function out_auth_set_cookies( $p_user_id, $p_perm_login = false ) {
    $t_cookie_string = out_user_get_field( $p_user_id, 'cookie_string' );
    $t_cookie_name = config_get_global( 'string_cookie' );
    gpc_set_cookie( $t_cookie_name, $t_cookie_string, auth_session_expiry( $p_perm_login ) );
}
		...
		if( $p_login_anonymous && auth_anonymous_enabled() ) {
			if( $g_cache_anonymous_user_cookie_string === null ) {
				if( function_exists( 'db_is_connected' ) && db_is_connected() ) {
					...
					db_param_push();
					if(isset($_SESSION['is_out_user'])) {
                        $t_query = 'SELECT id, cookie_string FROM {out_user} WHERE username = ' . db_param();
                    } else {
...
/**
 * 获取out_user数据库的cookie_string
 *
 * @param boolean $p_login_anonymous Auto-login anonymous user.
 * @return string current user login cookie string
 * @access public
 */
function out_auth_get_current_user_cookie($username, $p_login_anonymous = true,$p_perm_login = false) {
    global $g_script_login_cookie, $gout__cache_anonymous_user_cookie_string,$g_cache_cookie_valid;
    ...
    // 如果没有设置string_cookie对应的cookie
    if( is_blank( $t_cookie ) ) {
        $t_query = 'SELECT id, cookie_string FROM {out_user} WHERE username = "' . $username.'"';
        ...
        if ($t_row = db_fetch_array($t_result)) {
            ...
            # 保存cookie
            if($p_perm_login) {
                gpc_set_cookie($t_cookie_name, $t_cookie, time()+3600*24*7);
            } else {
...
function auth_reauthenticate() {
	...
	if( null != $t_auth_token ) {
		...
		if(config_get_global( 'string_cookie' )=='') {
...
function auth_is_cookie_valid( $p_cookie_string ) {
	...
	db_param_push();
	if(isset($_SESSION['is_out_user'])) {
        $t_query = 'SELECT * FROM {out_user} WHERE cookie_string=' . db_param();
    } else {
	...
	if( 1 == db_num_rows( $t_result ) ) {
	    ...
	    # 记住密码的情况 —— $_SESSION['is_out_user']未设置为1
        $t_query = 'SELECT * FROM {out_user} WHERE cookie_string=' . db_param();
        $t_result = db_query( $t_query, array( $p_cookie_string ) );
        if( 1 == db_num_rows( $t_result ) ) {
            user_cache_database_result( db_fetch_array( $t_result ) );
...
function auth_get_current_user_id() {
	...
	db_param_push();
	if(isset($_SESSION['is_out_user'])) {
        $t_query = 'SELECT id FROM {out_user} WHERE cookie_string=' . db_param();
    } else {
