 ...
 * @param $bug_id
 */
function bugnote_get_bug_text($bug_id) {
    $t_query = 'SELECT bt.note,b.date_submitted,b.reporter_id 
                  FROM mantis_bugnote_table AS b
	              LEFT JOIN mantis_bugnote_text_table AS bt 
	              ON bt.id = b.bugnote_text_id 
                  WHERE b.bug_id = '.$bug_id.' ORDER BY b.date_submitted desc';
    ...
    while( $t_row = db_fetch_array( $t_result ) ) {
        ...
        if(is_serialized($t_bugnote->note)) {
            # 反序列化数据
            $t_bugnote->note = unserialize($t_bugnote->note);
        } else {
            # 未封装过的数据
            # 将note转化为数组
            $t_bugnote->note = array('content' => $t_bugnote->note);
            $t_bugnote->note['time'] = date('Y-m-d h:i:s', $t_bugnote->date_submitted);
            $t_bugnote->note['is_pingjia'] = false;
            $t_bugnote->note['user_name'] = '工程师'.str_pad($t_bugnote->reporter_id,
                    5, '0', STR_PAD_LEFT).'号';
        }
