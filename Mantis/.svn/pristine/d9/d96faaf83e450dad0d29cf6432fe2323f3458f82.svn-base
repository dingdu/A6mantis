...
// $f_project_id
if($f_project_id!=''){
	$t_where.= ' and status =1 and project_id = '.$f_project_id;
}
...
$t_query = 'SELECT * FROM {supply_user_work} WHERE ' . $t_where . ' ORDER BY  status asc,' . $c_sort . ' ' . $c_dir;
...
while( $t_row = db_fetch_array( $t_result ) ) {
	...
	$t_query4= 'SELECT realname FROM {user} WHERE  id = '.db_param().' limit 1';
	$t_result4 = db_query( $t_query4,[$t_row['user_id']]);
	$t_results4 = db_fetch_array( $t_result4 );
	$t_row['realname'] = $t_results4['realname'];
...
<h4 class="widget-title lighter">
	...
	<?php echo lang_get('supply_manage_link') ?>
...
<?php
	...
	$t_columns = array(
	'by_users','project_name', 'bug_name','work_hours','reality_work_hours','view_bugs_log','supply_time',
		  'user_option'
	);
	...
	for( $i=0; $i<$t_user_count; $i++ ) {
		...
		if( !isset( $t_access_level[$u_access_level] ) ) {
			...
			<tr>
				...
				<td><?php echo  $t_user['description'] ?></td>
				<td><?php echo  date('Y-m-d',$t_user['add_time']) ?></td>
				<td><?php if($t_user['status']==1) { echo '<button class="btn-xs btn btn-primary" onclick="dring('.$t_user["supply_id"].')">处理</button>'; }else{ echo '已通过'; } ?>
				<?php if($t_user['status']==1) { echo '<button class="btn-xs btn btn-danger" onclick="del('.$t_user["supply_id"].')">删除</button>'; } ?></td>
			</tr>
<?php
	}  # end for
...
echo '<script>
    function dring(id){
      layer.confirm("确认同意补工时？", {
         btn: ["确认", "取消"] //可以无限个按钮
		}, function(index, layero){
		  //按钮【按钮一】的回调
           $.post("/supply_user_work_dring.php",{"id":id},function(data){
    ...
    function del(id){
      layer.confirm("确认删除？", {
         btn: ["确认", "取消"] //可以无限个按钮
		  ...
		  //按钮【按钮一】的回调
           $.post("/supply_user_work_dring.php?act=del",{"id":id},function(data){
                   data = JSON.parse(data);
                   if(data.status==1){
                       window.location.reload();
                   }else if(data.status==0){
