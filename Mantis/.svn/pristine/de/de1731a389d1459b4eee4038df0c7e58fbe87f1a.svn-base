...
foreach ( $t_summary_header_arr as $t_summary_header_name ) {
    if($daynow == $t_summary_header_name) $t_summary_header .= '<th class="align-left today">';
	else $t_summary_header .= '<th class="align-left">';
	$t_summary_header .= $t_summary_header_name;
	$t_summary_header .= '</th>';
}

layout_page_header( lang_get( 'by_user_month_work_hour_pv_count' ) );
...
$t_access_level = gpc_get_int( 'access_level',0);
...
<div class="widget-header widget-header-small">
	<h4 class="widget-title lighter">
		...
		<?php echo lang_get('by_user_month_work_hour_pv_count') ?>
	</h4>
</div>
...
<div class="widget-main no-padding">
    <span style="float:left;padding-left:15px;padding-top:5px;color:red;">统计方法: 当月已完成任务所花费标准工时/当月总标准工时X当月绩效系数（当月未完成的任务：状态不是已解决、已关闭则不计入绩效）</span>
    <form>
	<div class="col-md-6 col-xs-12  pull-right">
		 <div class="form-inline">
             <?php print_status_option_list('bug_status_level',$t_bug_status_level) ?>
				 <select id="user-access-level" name="access_level" class="input-sm">
					 <option value="0" selected="selected">全部</option>
					 <?php print_project_access_levels_option_list($t_access_level) ?>
				 </select>
			...
			<button type="submit" class="btn btn-primary btn-xs">搜 索</button>
			 <a class="btn btn-primary btn-white btn-round btn-sm" href="/user_work_hour_pv_csv_export.php?due_date=<?php echo $t_date_to_display?>&keyword=<?php echo $t_keyword?>&bug_status_level=<?php echo $t_bug_status_level?>&access_level=<?php echo $t_access_level?>">导出为Excel</a>
		</div>
...
<div class="col-md-12 col-xs-12">
	...
	<div class="widget-box table-responsive">
		...
		<?php summary_print_by_user_ware_hour_pv($t_date_to_display,$t_keyword,$t_access_level,$t_bug_status_level); ?>
	</table>
	</div>
