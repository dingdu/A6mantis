...
$t_result2 = db_query( $t_query2, array( $f_project_id ) );

// 获取用户等级 90 为管理员
//echo current_user_get_access_level();die;

$is_admin = current_user_get_access_level() == 90 ? true : false;
//var_dump($is_admin);die;

$t_query3 = 'SELECT username FROM  {user}  WHERE id=' . db_param();
$rew = db_fetch_array( $t_result2 );

$t_query_log = 'SELECT * FROM   {project_work_log}  WHERE project_id=' . db_param();
$t_project_log_res = db_query( $t_query_log, array( $f_project_id ) );
$t_project_log=[];
while( $t_log = db_fetch_array( $t_project_log_res ) ) {
	$un = db_fetch_array(db_query( $t_query3, array( $t_log['op_user_id'] ) ));
	$t_log['username'] =$un['username'];
	$t_project_log[] =$t_log;
}
array_multisort($t_project_log,SORT_DESC);

if($rew) {
	$t_row['ext'] =$rew;

	if($t_row['ext']['req_evaluate_user_id']>0){

		$t_result2 = db_query( $t_query3, array( $t_row['ext']['req_evaluate_user_id'] ) );

		$t_row['req_evaluate_user_id'] =  db_fetch_array($t_result2);
	}
	if($t_row['ext']['dev_evaluate_user_id']>0){
		$t_result2 = db_query( $t_query3, array( $t_row['ext']['dev_evaluate_user_id'] ) );
		$t_row['dev_evaluate_user_id'] =  db_fetch_array($t_result2);
	}
	if($t_row['ext']['test_evaluate_user_id']>0){
		$t_result2 = db_query( $t_query3, array( $t_row['ext']['test_evaluate_user_id'] ) );

		$t_row['test_evaluate_user_id'] =  db_fetch_array($t_result2);
	}
...
$f_handler_id	= empty($t_row['ext']['owner_user_id'])?0:(int)$t_row['ext']['owner_user_id'];
$f_req_evaluate_user_id	= empty($t_row['ext']['req_evaluate_user_id'])?0:(int)$t_row['ext']['req_evaluate_user_id'];
$f_test_evaluate_user_id	= empty($t_row['ext']['test_evaluate_user_id'])?0:(int)$t_row['ext']['test_evaluate_user_id'];
$f_dev_evaluate_user_id	= empty($t_row['ext']['dev_evaluate_user_id'])?0:(int)$t_row['ext']['dev_evaluate_user_id'];
	...
	<div class="table-responsive">
		...
		<fieldset>
            ...
            <tr>
                ...
                <td>
                    <input  type="text" id="proj-no" name="proj_no"maxlength="20" value="<?php echo string_attribute( $t_row['proj_no'] ) ?>"   />
                </td>
            </tr>
			<tr>
				<td class="category">
            ...
            <tr>
                ...
                <td>
                    <input id="sign_time" name="sign_time" type="text"
                           value="<?php if(!empty($rew['sign_time'])) echo date('Y-m-d', $rew['sign_time']); ?>">
                </td>
            </tr>
            ...
            <tr>
                ...
                <td>
                    <input id="submit_time" name="submit_time" type="text"
                           value="<?php if(!empty($rew['submit_time'])) echo date('Y-m-d', $rew['submit_time']); ?>" >
                </td>
            </tr>
            ...
            <tr>
				...
				<td>
					...
					<div class="input-group" style="width: 200px;float: left">
						...
						<input type="number"  min="0.1" step="0.1" <?php if(!$is_admin) echo 'disabled="disabled"'; ?>   id="need_working_hours_day" name="need_working_hours_day"  class="form-control"  value="<?php echo $t_row['ext']['req_work_hours']?>">
					...
					<select <?php echo helper_get_tab_index() ?> <?php if(!$is_admin) echo 'disabled="disabled"'; ?>  id="req_evaluate_user_id" name="req_evaluate_user_id" class="input-sm">
						<option value="0" selected="selected" ></option>
						<?php print_assign_to_option_list( $f_req_evaluate_user_id,ALL_PROJECTS) ?>
					</select>
			...
			<tr>
				...
				<td>
					...
					<div class="input-group" style="width: 200px;float: left">
						...
						<input type="number"  min="0.1" step="0.1" <?php if(!$is_admin) echo 'disabled="disabled"'; ?>   id="develop_working_hours_day" name="develop_working_hours_day"  class="form-control"  value="<?php echo $t_row['ext']['dev_work_hours']?>">
						<span class="input-group-addon"><?php echo lang_get( 'working_hours_day' ) ?></span>
					</div>
					<label class="label label-info" style="margin-left: 20px"><?php echo lang_get( 'dev_evaluate_user_id' ) ?></label>
					<select <?php echo helper_get_tab_index() ?> <?php if(!$is_admin) echo 'disabled="disabled"'; ?>  id="dev_evaluate_user_id" name="dev_evaluate_user_id" class="input-sm">
						<option value="0" selected="selected"></option>
						<?php print_assign_to_option_list( $f_dev_evaluate_user_id,ALL_PROJECTS) ?>
					</select>
			...
			<tr>
				...
				<td>
					...
					<div class="input-group" style="width: 200px;float: left">
						...
						<input type="number"   min="0.1" step="0.1"  <?php if(!$is_admin) echo 'disabled="disabled"'; ?> id="test_working_hours_day" name="test_working_hours_day"  class="form-control"  value="<?php echo $t_row['ext']['test_work_hours']?>">
						<span class="input-group-addon"><?php echo lang_get( 'working_hours_day' ) ?></span>
					</div>
					<label class="label label-info" style="margin-left: 20px"><?php echo lang_get( 'test_evaluate_user_id' ) ?></label>
					<select <?php echo helper_get_tab_index()  ?> <?php if(!$is_admin) echo 'disabled="disabled"'; ?>  id="test_evaluate_user_id" name="test_evaluate_user_id" class="input-sm">
						<option value="0" selected="selected" ></option>
						<?php print_assign_to_option_list( $f_test_evaluate_user_id,ALL_PROJECTS) ?>
					</select>
				</td>
			</tr>
            <tr>
                ...
                <td>
                    <div class="bonus-total-box">
                        <?php foreach($f_bonus_list as $bonus) { ?>
                            <!--把id放在clone外面-->
                            <div class="bonus-box" style="margin: 5px 10px;">
                                <input type="hidden" class="bonus_id" name="bonus_id[]"  <?php if(!$is_admin) echo 'disabled="disabled"'; ?> value="<?php echo $bonus['id'] ?>">
                                <label class="label label-info" >完成时间</label>
                                <input tabindex="1" value="<?php echo $bonus['deadline'] ?>" <?php if(!$is_admin) echo 'disabled="disabled"'; ?> type="text" id="due_date1" name="deadline[]" class="form-control fc-clear datetimepicker input-sm"
                                       data-picker-locale="zh-cn" data-picker-format="Y-MM-D" size="20" maxlength="16" style="" >
                                <label class="label label-info" >完成奖金</label>
                                <input type="number"  value="<?php echo $bonus['bonus'] ?>"  <?php if(!$is_admin) echo 'disabled="disabled"'; ?> name="bonus[]" min="0" >
                                <!-- 管理员才能增删改奖金 -->
                        ...
                        <?php if(empty($f_bonus_list)) { ?>
                            <div class="bonus-box" style="margin: 5px 10px;">
                                <label class="label label-info" >完成时间</label>
                                <input tabindex="1" type="text" id="due_date1" name="deadline[]"  <?php if(!$is_admin) echo 'disabled="disabled"'; ?> class="form-control fc-clear datetimepicker input-sm"
                                       data-picker-locale="zh-cn" data-picker-format="Y-MM-D" size="20" maxlength="16" style="" >
                                <label class="label label-info" >完成奖金</label>
                                <input type="number"  name="bonus[]" min="0"  <?php if(!$is_admin) echo 'disabled="disabled"'; ?>>
                                <?php if($is_admin == true) { ?>
                                    <button class="btn btn-info btn-xs del-bonus" style="margin-left:10px;padding: 2px 8px;border-radius: 5px;">删除</button>
                                    <button class="btn btn-info btn-xs add-bonus" style="margin-left:10px;padding: 2px 8px;border-radius: 5px;">添加</button>
                                <?php } ?>
...
?>
	...
	<div class="col-md-12 col-xs-12">
		<div class="space-10"></div>
		<div id="project_change_log" class="form-container">
			<div class="widget-box widget-color-blue2">
				<div class="widget-header widget-header-small">
					<h4 class="widget-title lighter">
						<i class="ace-icon fa fa-sitemap"></i>
						<?php echo lang_get( 'project_change_log' ); ?>
					</h4>
				</div>
				<div class="widget-toolbox padding-8 clearfix">
					...
					<div class="widget-main no-padding">
						...
						if( count( $t_project_log ) > 0 ) { ?>
							...
							<table class="table table-striped table-bordered table-condensed">
								...
								<tbody>
							<?php
							foreach ( $t_project_log as $t_val ) {
                                  $addtime = date('Y-m-d H:i:s',$t_val['add_time']);
								?>
								<tr>
									<td><?php echo $t_val['username']?></td>
									<td><?php echo $addtime;?></td>
									<td><?php echo $t_val['comment'];?></td>
								</tr>
...
?>
    <script>
        ...
        function addBonusBox(node) {
            ...
            /* 不要copy奖金 */
            copy.children('input.bonus_id').remove();
