...
//$f_is_inside         = gpc_get_int( 'is_inside' );
$f_skill_ratio         = gpc_get_string( 'skill_ratio' );
$f_user_type =   gpc_get_int( 'user_type',1);
$f_performance =   gpc_get_int( 'performance',60);
...
$t_data = array(
	...
	'payload' => array(
		...
		'enabled' => $f_enabled,
//		'skill_ratio'=>$f_skill_ratio,
//		'is_inside'=>$f_is_inside,
	)
);
...
// 添加 ext  mantis_user_ext_table

$t_query = 'INSERT INTO {user_ext}
					( user_id, skill, total_work_hours,user_type,performance)
				  VALUES
					( ' . db_param() . ', ' . db_param() . ', ' . db_param().', ' . db_param().', ' . db_param().')';
db_query( $t_query,['user_id'=>$t_user_id,'skill'=>$f_skill_ratio,'total_work_hours'=>0,'user_type'=>$f_user_type,'performance'=>$f_performance]);
