...
function filter_get_bug_rows( &$p_page_number, &$p_per_page, &$p_page_count, &$p_bug_count, $p_custom_filter = null, $p_project_id = null, $p_user_id = null, $p_show_sticky = null ) {
	...
	if(isset($_SESSION['is_out_user'])) {
	    # 设置报告者为自己
        $t_filter['reporter_id'] = array(auth_get_current_user_id());

        // 默认还是显示已关闭bug
        $t_filter['status'] = array(10, 20, 30, 40, 50, 80);
        # 用户默认只显示工单
        $t_filter['category_id'] = array('工单');
	    /*if(isset($_POST['category_id']) && !empty($_POST['category_id'])) {
            $t_filter['category_id'] = array(category_get_name($_POST['category_id']));
        } else {
	        $t_filter['category_id'] = array(0);
        }*/
        # 设置一个flag判断是否是post表单
        ...
        if(isset($_POST['summary']) && !empty($_POST['summary'])) {
            # 关键字保存到session中
            $_SESSION['filter_summary'] =  $_POST['summary'];
            $t_filter['search'] = $_POST['summary'];
        } else {
            # post时该字段为空才清空
            if($is_post) {
                $_SESSION['filter_summary'] = '';
            }
            $t_filter['search'] = '';
        }
        if(isset($_POST['status']) && !empty($_POST['status'])) {
            ...
            $t_filter['status'] = array($_POST['status']);
        } else {
            # post时该字段为空才清空
            if($is_post) {
                $_SESSION['filter_status'] =  0;
            }
            # 默认为0
            $t_filter['status'] = 0;
        }
        ...
        if(isset($_POST['startdate']) && !empty($_POST['startdate'])){
            # 标记是根据提交日期筛选
            $t_filter['filter_by_date'] = true;
            # 开始日期保存到session中
            $_SESSION['filter_startdate'] =  $_POST['startdate'];
            $timestamp = strtotime($_POST['startdate']);
            $t_filter['start_year'] = date('Y', $timestamp);
            $t_filter['start_month'] = date('m', $timestamp);
            $t_filter['start_day'] = date('d', $timestamp);
        } else {
            # post时该字段为空才清空
            if($is_post) {
                $_SESSION['filter_startdate'] =  '';
            }
            # 如果没有设置开始日期则默认2000-01-01
            $t_filter['start_year'] = '2000';
            $t_filter['start_month'] = '01';
            $t_filter['start_day'] = '01';
        }

        if(isset($_POST['enddate']) && !empty($_POST['enddate'])){
            $t_filter['filter_by_date'] = true;
            # 结束日期保存到session中
            $_SESSION['filter_enddate'] =  $_POST['enddate'];
            $timestamp = strtotime($_POST['enddate']);
            $t_filter['end_year'] = date('Y', $timestamp);
            $t_filter['end_month'] = date('m', $timestamp);
            $t_filter['end_day'] = date('d', $timestamp);
        } else {
            # post时该字段为空才清空
            if($is_post) {
                $_SESSION['filter_enddate'] = '';
            }
            # 如果没有设置结束时间则默认到现在
            $timestamp = time();
            $t_filter['end_year'] = date('Y', $timestamp);
    ...
    # 判断是否有post，没有则考虑分页（将过滤条件插进去）
    if(!$is_post) {
	    # 开始时间
	    if(isset($_SESSION['filter_startdate']) && !empty($_SESSION['filter_startdate'])) {
            $t_filter['filter_by_date'] = true;
            $timestamp = strtotime($_SESSION['filter_startdate']);
            $t_filter['start_year'] = date('Y', $timestamp);
            $t_filter['start_month'] = date('m', $timestamp);
            $t_filter['start_day'] = date('d', $timestamp);
        } else {
        ...
        # 结束时间
        if(isset($_SESSION['filter_enddate']) && !empty($_SESSION['filter_enddate'])) {
            $t_filter['filter_by_date'] = true;
            $timestamp = strtotime($_SESSION['filter_enddate']);
            $t_filter['end_year'] = date('Y', $timestamp);
            $t_filter['end_month'] = date('m', $timestamp);
            $t_filter['end_day'] = date('d', $timestamp);
        } else {
        ...
        # 状态
        if(isset($_SESSION['filter_status']) && !empty($_SESSION['filter_status'])) {
            $t_filter['status'] = array($_SESSION['filter_status']);
        } else {
            $t_filter['status'] = 0;
        }

        # 关键字
        if(isset($_SESSION['filter_summary']) && !empty($_SESSION['filter_summary'])) {
            $t_filter['search'] = $_SESSION['filter_summary'];
        }
    }

    // 设置每页为10行数据
    $t_filter['per_page'] = 10;
//    var_dump($t_filter);die;
...
function filter_get_bug_rows_filter( $p_project_id = null, $p_user_id = null ) {
	...
	if( null === $p_project_id ) {
        ...
        if(!isset($_SESSION['is_out_user'])) {
            ...
            // 客户直接从表中获取
            $t_project_id = out_user_get_field($t_user_id, 'project_id');
        }
	} else {
