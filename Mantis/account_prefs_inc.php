...
?>
	...
	<?php if( ON == config_get( 'enable_email_notification' ) ) { ?>
        ...
        <tr>
            <td class="category">
                <?php echo '当问题将要开始时发送Email提醒'; ?>
            </td>
            <td>
                <label class="inline">
                    <input type="checkbox" class="ace input-sm" id="email_on_tostart" name="email_on_tostart" <?php check_checked( (int)$t_pref->email_on_tostart, ON ); ?> />
                    <span class="lbl"></span>
                </label>
                <label for="email_on_tostart_min_severity" class="email-on-severity-label"><span><?php echo lang_get( 'with_minimum_severity' ) ?></span></label>
                <select id="email_on_tostart_min_severity" name="email_on_tostart_min_severity" class="input-sm">
                    <option value="<?php echo OFF ?>"><?php echo lang_get( 'any' ) ?></option>
                    <option disabled="disabled">-----</option>
                    <?php print_enum_string_option_list( 'severity', (int)$t_pref->email_on_tostart_min_severity ) ?>
                </select>
                <label style="margin: 0 5px 0 25px;" class="email-on-severity-label"><span>提前</span></label>
                <input value="<?php echo (int)$t_pref->email_on_tostart_hours_in_advance; ?>" style="height: 30px; width: 80px;" type="number" id="email_on_tostart_hours_in_advance" name="email_on_tostart_hours_in_advance"  min="0" step="1" class="form-control">
                <label style="margin: 0 5px;" for="" class="email-on-severity-label"><span>小时</span></label>
            </td>
        </tr>

        <tr>
            <td class="category">
                <?php echo '当问题将要结束时发送Email提醒'; ?>
            </td>
            <td>
                <label class="inline">
                    <input type="checkbox" class="ace input-sm" id="email_on_toend" name="email_on_toend" <?php check_checked( (int)$t_pref->email_on_toend, ON ); ?> />
                    <span class="lbl"></span>
                </label>
                <label for="email_on_toend_min_severity" class="email-on-severity-label"><span><?php echo lang_get( 'with_minimum_severity' ) ?></span></label>
                <select id="email_on_toend_min_severity" name="email_on_toend_min_severity" class="input-sm">
                    <option value="<?php echo OFF ?>"><?php echo lang_get( 'any' ) ?></option>
                    <option disabled="disabled">-----</option>
                    <?php print_enum_string_option_list( 'severity', (int)$t_pref->email_on_toend_min_severity ) ?>
                </select>
                <label style="margin: 0 5px 0 25px;" class="email-on-severity-label"><span>提前</span></label>
                <input value="<?php echo (int)$t_pref->email_on_toend_hours_in_advance; ?>" style="height: 30px; width: 80px;" type="number" id="email_on_toend_hours_in_advance" name="email_on_toend_hours_in_advance"  min="0" step="1" class="form-control">
                <label style="margin: 0 5px;" for="" class="email-on-severity-label"><span>小时</span></label>
            </td>
        </tr>
        <!--自定义邮件提醒功能设置结束-->
