...
$t_updated_bug->expected_starttime = gpc_get_string('expected_starttime', date('Y-m-d h:i:s', $t_existing_bug->expected_starttime));
$t_updated_bug->expected_endtime = gpc_get_string('expected_endtime', date('Y-m-d h:i:s', $t_existing_bug->expected_endtime));

// 转为时间戳
if(is_null($t_updated_bug->expected_starttime)) {
    $t_updated_bug->expected_starttime = null;
} else {
    $t_updated_bug->expected_starttime = strtotime($t_updated_bug->expected_starttime);
}

if(is_null($t_updated_bug->expected_endtime)) {
    $t_updated_bug->expected_endtime = null;
} else {
    $t_updated_bug->expected_endtime = strtotime($t_updated_bug->expected_endtime);
}
...
// 获取修改前的状态
$status = bug_get_field($t_updated_bug->id, 'status');

$t_updated_bug->update( $t_text_field_update_required, true );

$category_name = category_get_name($t_updated_bug->category_id);
// 状态变为完成
if($t_updated_bug->status == 80 & $status != 80 && $category_name == '工单') {
    // 这里要隐藏报错信息
    @notifySolveBugsToOutUser($t_updated_bug->reporter_id, $t_updated_bug->id);
}
