...
class BugData {
     ...
     * 新增字段： 预计开始时间
     */
    public $expected_starttime = 'null';

    /**
     * 新增字段： 预计结束时间
     */
    public $expected_endtime = 'null';

    /**
     * Notes: 根据bug的关联数组来初始化Bugdata
     * User: dingduming
     * Date: 2018\8\1 0001
     * @param $arr（除了bug之外还要有description）
     */
    public function init_data_by_arr($arr)  {
        $this->project_id = $arr['project_id'];
        $this->reporter_id = $arr['reporter_id'];
        $this->handler_id = $arr['handler_id'];
        $this->duplicate_id = $arr['duplicate_id'];
        $this->priority = $arr['priority'];
        $this->severity = $arr['severity'];
        $this->reproducibility = $arr['reproducibility'];
        $this->resolution = $arr['resolution'];
        $this->projection = $arr['projection'];
        $this->eta = $arr['eta'];
        // 创建问题时 bug_text是重新获取的
        $this->bug_text_id = $arr['status'];
        $this->os = $arr['os'];
        $this->os_build = $arr['os_build'];
        $this->platform = $arr['platform'];
        $this->version = $arr['version'];
        $this->fixed_in_version = $arr['fixed_in_version'];
        $this->build = $arr['build'];
        $this->profile_id = $arr['profile_id'];
        $this->view_state = $arr['view_state'];
        $this->summary = $arr['summary'];
        $this->sponsorship_total = $arr['sponsorship_total'];
        $this->sticky = $arr['sticky'];
        $this->target_version = $arr['target_version'];
        $this->category_id = $arr['category_id'];
        $this->date_submitted = $arr['date_submitted'];
        $this->due_date = $arr['due_date'];
        $this->last_updated = $arr['last_updated'];
        $this->category_id = $arr['category_id'];
        $this->evaluate_time = $arr['evaluate_time'];
        $this->expected_starttime = $arr['expected_starttime'];
        $this->expected_endtime = $arr['expected_endtime'];
        // bug_text表的内容
	...
	function create() {
		...
		// 您已经达到了可允许的活动极限 - 10 次事件，3600 秒内；您的行为已经被阻止以防止垃圾信息，请稍候再试。
//		antispam_check();
		...
		$t_query = 'INSERT INTO {bug}
					    ( project_id,reporter_id, handler_id,duplicate_id,
					      ...
					      target_version, due_date,evaluate_time,expected_starttime,expected_endtime
					    )
					  VALUES
					    ( ' . db_param() . ',' . db_param() . ',' . db_param() . ',' . db_param() . ',
					      ...
					      ' . db_param() . ',' . db_param() . ',' . db_param() . ',' . db_param() . ',
					      ' . db_param() . ','.  db_param() . ',' .  db_param() . ')';
//		var_dump($this->expected_starttime, $this->expected_endtime);die;
		db_query( $t_query, array( $this->project_id, $this->reporter_id, $this->handler_id, $this->duplicate_id, $this->priority, $this->severity, $this->reproducibility, $t_status, $this->resolution, $this->projection, $this->category_id, $this->date_submitted, $this->last_updated, $this->eta, $t_text_id, $this->os, $this->os_build, $this->platform, $this->version, $this->build, $this->profile_id, $this->summary, $this->view_state, $this->sponsorship_total, $this->sticky, $this->fixed_in_version, $this->target_version, $this->due_date,$this->evaluate_time, $this->expected_starttime, $this->expected_endtime ) );
	...
	function update( $p_update_extended = false, $p_bypass_mail = false ) {
		...
		$t_query = 'UPDATE {bug}
					SET project_id=' . db_param() . ', reporter_id=' . db_param() . ',
						...
						expected_starttime='. db_param() . ',expected_endtime='. db_param() . ',';
		...
		$t_fields = array(
			...
			$this->expected_starttime, $this->expected_endtime,
		);
		...
		history_log_event_direct( $c_bug_id, '预计开始时间', date('Y-m-d H:i:s', $t_old_data->expected_starttime), date('Y-m-d H:i:s', $this->expected_starttime) );
		history_log_event_direct( $c_bug_id, '预计结束时间', date('Y-m-d H:i:s', $t_old_data->expected_endtime), date('Y-m-d H:i:s', $this->expected_endtime) );
 ...
 * @param $project_id
 */
function bug_get_rows_by_project($project_id) {
    // 过滤掉关闭问题
    $t_query = 'SELECT c.name AS category_name,summary,u.realname,expected_starttime,expected_endtime,b.status,priority FROM {bug} AS b 
      LEFT JOIN {user} AS u ON b.handler_id = u.id
      LEFT JOIN {category} AS c ON c.id = b.category_id 
      WHERE b.status < 90 AND b.project_id='.$project_id .'
 ...
 * @param $bug_id
 */
function bug_hasjiaji($bug_id) {
//    $t_query = 'SELECT b.id FROM {bugnote} AS b
//                  LEFT JOIN {bugnote_text} AS bt
//                  ON bt.id=b.bugnote_text_id
//                  AND bt.note="加急"
//                  WHERE b.bug_id='.$bug_id;
    $t_query = 'SELECT id FROM {bug} WHERE priority >= 50 AND id='.$bug_id;
    $t_result = db_query( $t_query);
...
/**
 * Notes:判断这个问题是当前用户的工单
 * User: dingduming
 * Date: 2018\7\18 0018
 * Time: 16:42
 * @param $bug_id
 */
function is_current_out_users_gongdan() {

}

/**
 * Notes: 判断bug是否已评价（已解决）
 * User: dingduming
 * Date: 2018\7\21 0021
 * Time: 10:18
 */
function bug_has_pingjia_is_solved($bug_id) {
    # 先获取bug所有对应的注释
    $t_bugnotes = bugnote_get_bug_text($bug_id);
    $flag = false;
    # 遍历判断
    foreach($t_bugnotes as $t_bugnote) {
        // 用户表明问题解决并且评价过了
        if($t_bugnote->note['is_pingjia'] && $t_bugnote->note['isSolve'] == 1) {
            $flag = true;
            break;
        }
    }
