...
function print_user_count() {
	# Plugin / Event added options
	$t_event_menu_options = event_signal( 'EVENT_USER_COUNT' );
	$t_menu_options = array();
	foreach( $t_event_menu_options as $t_plugin => $t_plugin_menu_options ) {
		foreach( $t_plugin_menu_options as $t_callback => $t_callback_menu_options ) {
			if( is_array( $t_callback_menu_options ) ) {
...
function print_manage_menu( $p_page = '' ) {
	...
	if( access_has_global_level( config_get( 'manage_user_threshold' ) ) ) {
		$t_pages['manage_out_user_page.php'] = array( 'url'   => 'manage_out_user_page.php', 'label' => 'manage_out_users_link' );
	}
	if( access_has_project_level( config_get( 'manage_project_threshold' ) ) ) {
		$t_pages['manage_proj_page.php'] = array( 'url'   => 'manage_proj_page.php', 'label' => 'manage_projects_link' );
	}
    // 添加模版类别编辑
    if( access_has_project_level( config_get( 'manage_template_threshold' ) ) ) {
        $t_pages['manage_template_threshold.php'] = array( 'url'   => 'manage_template_page.php', 'label' => 'manage_template_link' );
    }
