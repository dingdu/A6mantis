...
function layout_navbar() {
	...
	// 提交问题按钮 href = bug_report_page.php
	...
	if (auth_is_user_authenticated()) {
	    if(!isset($_SESSION['is_out_user'])) {
            # shortcuts button bar 下拉按钮
            layout_navbar_button_bar();
            # projects dropdown menu 下拉项目菜单
            layout_navbar_projects_menu();
        }

		# user buttons such as messages, notifications and user menu 下拉用户菜单
...
function layout_navbar_user_menu( $p_show_avatar = true ) {
    ...
    if(!empty($GLOBALS['is_out_user'])) {
	    $id = auth_get_current_user_id();
        $t_username = out_user_get_field($id, 'username');
    } else {
	...
	if( !current_user_is_protected() ) {
        if(!isset($_SESSION['is_out_user'])) {
            ...
            layout_navbar_menu_item(helper_mantis_url('out_account_page.php'), lang_get('account_link'), 'fa-user');
        }
	}
...
function layout_print_sidebar( $p_active_sidebar_page = null ) {
	if( auth_is_user_authenticated() ) {
        ...
        if(!isset($_SESSION['is_out_user'])) {
            ...
            // 客户的工单页面
            layout_sidebar_menu( 'view_all_bug_page.php', '工单列表', 'fa-list-alt', $p_active_sidebar_page );
        }
        ...
        #客户端提问
        if(isset($_SESSION['is_out_user'])) {
            layout_sidebar_menu( 'bug_report_page.php', '提交工单', 'fa-edit', $p_active_sidebar_page );
        } else if( access_has_any_project_level( 'report_bug_threshold' ) ) {
			$t_bug_url = string_get_bug_report_url();
        ...
        if(!isset($_SESSION['is_out_user'])) {
            ...
            if( access_has_any_project_level( 'report_bug_threshold' ) ) {
                $t_supply_url = 'supply_page.php';
                layout_sidebar_menu( $t_supply_url, 'supply_link', 'fa-edit', $p_active_sidebar_page );
            }

            #补给管理
            if( access_has_any_project_level( 'supply_manage_threshold_min' ) ) {
                $t_supply_url = 'supply_manage_page.php';
                layout_sidebar_menu( $t_supply_url, 'supply_manage_link', 'fa-edit', $p_active_sidebar_page );
            }
...
//			layout_sidebar_menu( 'roadmap_page.php', 'roadmap_link', 'fa-road', $p_active_sidebar_page );
            ...
            layout_sidebar_menu( 'user_count_page.php', 'user_count', 'fa-bar-chart-o', $p_active_sidebar_page );

            # 统计
            if( access_has_project_level( config_get( 'view_summary_threshold' ), $t_current_project ) ) {
                layout_sidebar_menu( 'summary_count_page.php', 'summary_link', 'fa-bar-chart-o', $p_active_sidebar_page );
            }
...
function layout_breadcrumbs() {
	...
	if( current_user_is_anonymous() ) {
		...
		if(isset($_SESSION['is_out_user'])) {
            $t_access_level = '客户';
            $t_page = ( OFF == $t_protected ) ? 'out_account_page.php' : 'my_view_page.php';
        } else {
...
function layout_footer() {
	...
	echo '<address>' . "\n";
	echo '<strong>Powered by <a href="https://www.a6shop.com" title="bug tracking software">A6shop ' . $t_version_suffix . '</a></strong> <br>' . "\n";
	echo "<small>Copyright &copy;$t_copyright_years A6shop Team</small>" . '<br>';
	...
	if( !is_page_name( 'login_page' ) ) {
		...
		echo '<a href="https://www.a6shop.com" '.
			'title="A6shop Bug Tracker.">' .
			'<img src="' . $t_mantisbt_logo_url . '" width="102" height="35" ' .
			'alt="Powered by A6shop Bug Tracker" />' .
			'</a>' . "\n";
		echo '</div>' . "\n";
		echo '</div>' . "\n";
	}
