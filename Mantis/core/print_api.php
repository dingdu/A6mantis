...
/**
 * Notes: 显示相应的错误（但是立即停止）
 * Notice: 调用该方法需要先调用layout_page_header()才能有样式
 * User: dingduming
 * Date: 2018\7\19 0019
 * Time: 11:23
 * @param $error_string
...
function print_error($error_string) {
    ...
    echo '<p class="center">' . $error_string . '</p><p class="center">';
...
/**
 * Notes: 显示相应内容的错误报告页面（带头部、侧边、底部）
 * User: dingduming
 * Date: 2018\7\19 0019
 * Time: 11:30
 * @param $error_string
 */
function print_error_page($error_string) {
    # 显示错误报告页面
    layout_page_header();
 ...
 * @return void
 */
function print_user( $p_user_id, $bug_id=0 ) {
	echo prepare_user_name( $p_user_id, $bug_id );
}
 ...
 * @return void
 */
function print_user_with_subject( $p_user_id, $p_bug_id, $isReport=true) {
	if( NO_USER == $p_user_id ) {
		return;
	}
	# 判断bug是否是工单类型
    $category_id = bug_get_field($p_bug_id, 'category_id');
	if($category_id == 15 && $isReport) {
        $t_username = out_user_get_username( $p_user_id );
        $t_name = out_user_get_name( $p_user_id );
    } else {
        $t_username = user_get_username( $p_user_id );
        $t_name = user_get_name( $p_user_id );
    }
    # 分别处理工单用户和后台用户
    if( $category_id == 15 &&  out_user_exists( $p_user_id ) && out_user_get_field( $p_user_id, 'enabled' )) {
        $t_email = out_user_get_email( $p_user_id );
        print_email_link_with_subject( $t_email, $t_username, $t_name, $p_bug_id );
    } else if(user_exists( $p_user_id ) && user_get_field( $p_user_id, 'enabled' )) {
...
function print_user_option_list( $p_user_id, $p_project_id = null, $p_access = ANYBODY ) {
	...
	for( $i = 0;$i < $t_count;$i++ ) {
		...
		echo '>' . string_attribute( $t_display[$i] ) . '('.$t_row['realname'].')'.'</option>';
	}
}
