 ...
 * @return integer The number of users.
 */
function user_today_ware_hours($user_id,$sid=''){
	db_param_push();
	if($sid!=''){
	...
	$t_query = 'SELECT sum(work_hours) FROM {user_work_log} WHERE user_id = ' . db_param()." and add_time>={$startTime} and  add_time<={$endTime}";
	$op_user_id = $user_id;
	# Get the number of users
	$t_result = db_query( $t_query, [$op_user_id] );
	$t_count = db_result( $t_result );
     if($t_count==''){
		 $t_count =0;
	 }
 ...
 * @return string Cookie String
 */
function out_user_create( $p_username, $p_password, $p_project_id, $p_email = '',
                      $p_access_level = null, $p_protected = false, $p_enabled = true,
                      $p_realname = '', $p_admin_name = '' ) {
    if( null === $p_access_level ) {
    ...
    # Create preferences for the user
    $t_user_id = db_insert_id( db_get_table( 'out_user' ) );
 ...
 * @return string Cookie String
 */
function out_user_update($t_user_id, $p_username, $p_password,  $p_project_id, $p_email = '',
                          $p_access_level = null, $p_protected = false, $p_enabled = true,
                          $p_realname = '', $p_admin_name = '' ) {
    if( null === $p_access_level ) {
        $p_access_level = config_get( OUTWORKER );
    ...
    $c_enabled = (bool)$p_enabled;
    user_ensure_name_valid( $p_username );
    user_ensure_name_unique( $p_username, $t_user_id);
    user_ensure_email_unique( $p_email, $t_user_id);
    email_ensure_valid( $p_email );
    email_ensure_not_disposable( $p_email );
    $t_cookie_string = auth_generate_unique_cookie_string();
    db_param_push();
    ...
    if(!is_blank($p_password)){
        ...
        $t_query = 'UPDATE {out_user} SET username=' . db_param() . ', email=' . db_param() . ', password=' . db_param() . ', project_id=' . db_param() . ', enabled=' . db_param() .
            ...
            ' WHERE id=' . db_param();
        db_query( $t_query, array( $p_username, $p_email, $t_password, $p_project_id, $c_enabled, (int)$p_access_level, $t_cookie_string, $p_realname,$t_user_id) );
    }else{
        $t_query = 'UPDATE {out_user} SET username=' . db_param() . ', email=' . db_param() . ', project_id=' . db_param() . ', enabled=' . db_param() .
            ',access_level=' . db_param() . ',cookie_string=' . db_param() .',realname=' . db_param() .
            ' WHERE id=' . db_param();
        db_query( $t_query, array( $p_username, $p_email, $p_project_id, $c_enabled, (int)$p_access_level, $t_cookie_string, $p_realname,$t_user_id) );
    }
...
function user_delete( $p_user_id ) {
	...
	#Remove ext
	$t_query = 'DELETE FROM {user_ext} WHERE user_id=' . db_param();
	db_query( $t_query, array( $c_user_id ) );
 ...
 * @return array
 */
function user_get_accessible_projects( $p_user_id, $p_show_disabled = false,  $search_condition = array() ) {
	global $g_user_accessible_projects_cache;
	$p_proj_name = (isset($search_condition['proj_name']) ? $search_condition['proj_name'] : '');
	$p_proj_no = (isset($search_condition['proj_no']) ? $search_condition['proj_no'] : '');
	...
	if( access_has_global_level( config_get( 'private_project_threshold' ), $p_user_id ) ) {
		$t_projects = project_hierarchy_get_subprojects( ALL_PROJECTS, $p_show_disabled, $search_condition );
	} else {
		$t_public = VS_PUBLIC;
						  ...
						  WHERE ' . ( $p_show_disabled ? '' : ( 'p.enabled = ' . db_param() . ' AND ' ) ) . '
							( p.view_state=' . db_param() . '
							    OR (p.view_state=' . db_param() . '
								    AND
							        u.user_id=' . db_param() . ' )
							) AND p.name like "%'. $p_proj_name .'%" 
                            AND p.proj_no like "%'. $p_proj_no .'%" 
                            ORDER BY p.name';
