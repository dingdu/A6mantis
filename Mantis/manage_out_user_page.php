...
foreach ( $t_prefix_array as $t_prefix => $t_caption ) {
	...
	$t_active = (string)$t_prefix === $f_filter ? 'active' : '';
		print_manage_user_sort_link( 'manage_out_user_page.php',
			$t_caption,
			$c_sort,
...
$t_query = 'SELECT count(*) as user_count FROM {out_user} WHERE ' . $t_where;
...
$t_query = 'SELECT * FROM {out_user} WHERE ' . $t_where . ' ORDER BY ' . $c_sort . ' ' . $c_dir;
$t_result = db_query( $t_query, $t_where_params, $p_per_page, $t_offset );

$t_users = array();
while( $t_row = db_fetch_array( $t_result ) ) {
    // 获取客户对应的项目的负责人
    $t_query4 = 'SELECT p.name AS project_name,fz.id,fz.username,fz.realname 
          FROM {out_user} AS u 
          LEFT JOIN {project} AS p
          ON p.id = u.project_id
          LEFT JOIN {project_ext} AS pe  
          ON u.project_id=pe.project_id
          LEFT JOIN {user} AS fz
          ON fz.id=pe.owner_user_id
          WHERE u.id='. $t_row['id'];
    $t_result4 = db_query($t_query4);
    $ress = db_fetch_array($t_result4);
    // 输出客户对应负责人
    if($ress) {
        $t_row['owner_user'] = $ress;
    }


    $t_query3 = 'SELECT skill,total_work_hours FROM {user_ext} WHERE  user_id = '.db_param();
	$t_result3 = db_query( $t_query3, array($t_row['id']) );
	$t_row['ext']['skill'] = 0;
	$t_row['ext']['total_work_hours'] = 0;
	$ress = db_fetch_array($t_result3);
	if($ress) {
		$t_row['ext'] = $ress ;
	}
...
<div class="widget-toolbox padding-8 clearfix">
	<div id="manage-user-div" class="form-container">
		<div class="pull-left">
			<?php print_form_button( 'manage_out_user_create_page.php', lang_get( 'create_new_out_account_link' ), null, null, 'btn btn-primary btn-white btn-round' ) ?>
		</div>
		<?php if( $f_filter === 'UNUSED' ) { ?>
		<div class="pull-left">
			<?php print_form_button('manage_out_user_prune.php', lang_get('prune_accounts'), null, null, 'btn btn-primary btn-white btn-round') ?>
		</div>
		<?php } ?>
	<div class="pull-right">
	<form id="manage-user-filter" method="post" action="manage_out_user_page.php" class="form-inline">
		<fieldset>
...
<?php
	...
	$t_columns = array(
        ...
        // 去除保护两个列，新增项目名称和项目负责人列
//		'enabled', 'protected',
        'date_created', 'last_visit','project_name', 'projects_fuzr'
	);
			...
			<tr>
				<td><?php
					if( access_has_global_level( $u_access_level ) ) { ?>
						<a href="manage_out_user_edit_page.php?user_id=<?php echo $u_id ?>"><?php echo string_display_line( $u_username ) ?></a><?php
					} else {
...
/*					if( $u_protected ) {
				...
				<td><?php echo  $t_user['owner_user']['project_name'] ?></td>
<!--				<td>--><?php //echo $t_user['ext']['skill']?><!--</td>-->
                <td><a href ="/view_user_page.php?user_id=<?php echo $t_user['owner_user']['id'] ?>"><?php echo $t_user['owner_user']['username'].'('.$t_user['owner_user']['realname'].')' ?></a></td>
			</tr>
<?php
	}  # end for
?>
...
<div class="widget-toolbox padding-8 clearfix">
        ...
        <div class="btn-toolbar pull-right">
            ...
            print_page_links( 'manage_out_user_page.php', 1, $t_page_count, (int)$f_page_number, $f_filter . $t_hide_inactive_filter . $t_show_disabled_filter . "&amp;sort=$c_sort&amp;dir=$c_dir");
            ?>
        </div>
</div>
</div>
