...
<?php
	...
	$t_columns = array(
		...
		'enabled', 'protected', 'date_created', 'last_visit','skill_ratio','total_work_house','user_option'
	);
	...
	for( $i=0; $i<$t_user_count; $i++ ) {
		...
		if( !isset( $t_access_level[$u_access_level] ) ) {
			...
			<tr>
				...
				<td><?php echo $t_user['ext']['total_work_hours']?></td>
				<td>
					<a href="/user_work_log_list_page.php?user_id=<?php echo $t_user['id']?>"><button class="btn btn-primary btn-xs">今天日志</button></a>
					&nbsp;
					<a href="/user_work_log_list_page.php?type=all&user_id=<?php echo $t_user['id']?>"><button class="btn btn-danger btn-xs">历史日志</button></a>
				</td>
			</tr>
<?php
	}  # end for
