...
if($t_user['id']!=''){
  	$sql = "select skill,user_type,performance from {user_ext} where user_id = ".db_param();
    $t_result = db_query($sql,[$t_user['id']]);
    $t_row = db_fetch_array($t_result);
    if($t_row!=''){
    $t_user['skill'] =$t_row['skill'];
    }else{
     $t_user['skill'] = 0;
    }
    $user_type =$t_row['user_type'];
    $t_user['performance'] =$t_row['performance'];
}
...
<div id="edit-user-div" class="form-container">
	<form id="edit-user-form" method="post" action="manage_user_update.php">
		...
		?>
			...
			<tr>
				<td class="category">
					<?php echo lang_get( 'skill_ratio' ) ?>
				</td>
				<td>
					<input type="number" required="required" min="0.01" step="0.01"   name="skill_ratio" class="form-control input-sm"  value="<?php echo $t_user['skill']?>"/>
				</td>
				</tr>
            ...
            <tr>
				...
				<tr>
				<td class="category">
					<?php echo lang_get( 'performance_score' ) ?>
				</td>
				<td>
					<input type="number" required="required" min="0.01" step="0.01"   name="performance" class="form-control input-sm"  value="<?php echo $t_user['performance']?>"/>
				</td>
				</tr>
