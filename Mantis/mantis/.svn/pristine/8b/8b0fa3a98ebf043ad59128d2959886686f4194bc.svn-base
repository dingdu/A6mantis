...
/**
 * Print row with percentage in user_count table
 *
 * @param string $p_label    The user_count row label.
 ...
 * @return void
 */
function user_count_helper_print_row( $p_label, $p_open, $p_resolved, $p_closed, $p_total, $p_resolved_ratio, $p_ratio,$p_ware_hours=0) {
	echo '<tr>';
 ...
 * @return string
 */
function user_count_helper_get_developer_label( $p_user_id ) {
 ...
 * @return void
 */
function user_count_helper_build_bugcount( &$p_cache, $p_key, $p_status, $p_bugcount ) {
 ...
 * @return void 
 */
function user_count_helper_build_buglinks( $p_bug_link, &$p_bugs_open, &$p_bugs_resolved, &$p_bugs_closed, &$p_bugs_total) {
	$t_resolved_val = config_get( 'bug_resolved_status_threshold' );
	$t_closed_val = config_get( 'bug_closed_status_threshold' );
 ...
 */
function user_count_helper_get_bugratio( $p_bugs_open, $p_bugs_resolved, $p_bugs_closed, $p_bugs_total_count) {
...
/**
 * Used in user_count reports - this function prints out the user_count for the given enum setting
 * The enum field name is passed in through $p_enum
 *
 * @param string $p_enum Enum field name.
 * @return void
 */
function user_count_print_by_enum( $p_enum ) {
	...
	while( $t_row = db_fetch_array( $t_result ) ) {
		...
		user_count_helper_build_bugcount( $t_cache, $t_enum, $t_status, $t_bugcount );
	}
	...
	foreach( $t_cache as $t_enum => $t_item) {
		...
		$t_bugs_ratio = user_count_helper_get_bugratio( $t_bugs_open, $t_bugs_resolved, $t_bugs_closed, $t_bugs_total_count);
		...
		user_count_helper_print_row( get_enum_element( $p_enum, $t_enum ), $t_bugs_open, $t_bugs_resolved, $t_bugs_closed, $t_bugs_total, $t_bugs_ratio[0], $t_bugs_ratio[1] );
	}
}
 ...
 * @return integer
 */
function user_count_new_bug_count_by_date( $p_num_days = 1 ) {
 ...
 * @return integer
 */
function user_count_resolved_bug_count_by_date( $p_num_days = 1 ) {
 ...
 * @return void
 */
function user_count_print_by_date( array $p_date_array ) {
	foreach( $p_date_array as $t_days ) {
		$t_new_count = user_count_new_bug_count_by_date( $t_days );
		$t_resolved_count = user_count_resolved_bug_count_by_date( $t_days );
 ...
 * @return void
 */
function user_count_print_by_activity() {
	$t_project_id = helper_get_current_project();
	$t_resolved = config_get( 'bug_resolved_status_threshold' );
	...
	$t_query = 'SELECT COUNT(h.id) as count, b.id, b.user_count, b.view_state
				FROM {bug} b, {bug_history} h
				WHERE h.bug_id = b.id
				AND b.status < ' . db_param() . '
				AND ' . $t_specific_where . '
				GROUP BY h.bug_id, b.id, b.user_count, b.last_updated, b.view_state
				ORDER BY count DESC, b.last_updated DESC';
	$t_result = db_query( $t_query, array( $t_resolved ) );

	$t_count = 0;
	$t_private_bug_threshold = config_get( 'private_bug_threshold' );
	$t_user_countdata = array();
	$t_user_countbugs = array();
	while( $t_row = db_fetch_array( $t_result ) ) {
		...
		$t_user_countdata[] = array(
			'id' => $t_row['id'],
			'user_count' => $t_row['user_count'],
			'count' => $t_row['count'],
		);
		$t_user_countbugs[] = $t_row['id'];
	}

	bug_cache_array_rows( $t_user_countbugs );

	foreach( $t_user_countdata as $t_row ) {
		$t_bugid = string_get_bug_view_link( $t_row['id'], false );
		$t_user_count = string_display_line( $t_row['user_count'] );
		$t_notescount = $t_row['count'];

		echo '<tr>' . "\n";
		echo '<td class="small">' . $t_bugid . ' - ' . $t_user_count . '</td><td class="align-right">' . $t_notescount . '</td>' . "\n";
		echo '</tr>' . "\n";
	}
}
 ...
 * @return void
 */
function user_count_print_by_age() {
	$t_project_id = helper_get_current_project();
	$t_resolved = config_get( 'bug_resolved_status_threshold' );
	...
	while( $t_row = db_fetch_array( $t_result ) ) {
		...
		echo '<tr>' . "\n";
		echo '<td class="small">' . $t_bugid . ' - ' . $t_user_count . '</td><td class="align-right">' . $t_days_open . '</td>' . "\n";
		echo '</tr>' . "\n";
	}
}
 ...
 * @return void
 */
function user_count_print_by_developer() {
	$t_project_id = helper_get_current_project();
	...
	$t_user_countusers = array();
	$t_cache = array();
	$t_bugs_total_count = 0;

	while( $t_row = db_fetch_array( $t_result ) ) {
		$t_user_countusers[] = $t_row['handler_id'];
		$t_status = $t_row['status'];
		$t_bugcount = $t_row['bugcount'];
		$t_bugs_total_count += $t_bugcount;
		$t_label = $t_row['handler_id'];

		user_count_helper_build_bugcount( $t_cache, $t_label, $t_status, $t_bugcount );
	}
	
	user_cache_array_rows( array_unique( $t_user_countusers ) );
	...
	foreach( $t_cache as $t_label => $t_item) {
		...
		$t_label = user_count_helper_get_developer_label( $t_label );
		user_count_helper_build_buglinks( $t_bug_link, $t_bugs_open, $t_bugs_resolved, $t_bugs_closed, $t_bugs_total );
		user_count_helper_print_row( $t_label, $t_bugs_open, $t_bugs_resolved, $t_bugs_closed, $t_bugs_total, $t_bugs_ratio[0], $t_bugs_ratio[1] );
	}
}
 ...
 * @return void
 */
function user_count_print_by_reporter() {
	$t_reporter_user_count_limit = config_get( 'reporter_user_count_limit' );
	...
	$t_query = 'SELECT reporter_id, COUNT(*) as num
				...
				ORDER BY num DESC';
	$t_result = db_query( $t_query, array(), $t_reporter_user_count_limit );
	...
	foreach( $t_reporters as $t_reporter ) {
		...
		if( 0 < $t_bugs_total ) {
			...
			user_count_helper_print_row( $t_user, $t_bugs_open, $t_bugs_resolved, $t_bugs_closed, $t_bugs_total, $t_bugs_ratio[0], $t_bugs_ratio[1] );
		}
	}
}
 ...
 * @return void
 */
function user_count_print_by_category() {
	$t_user_count_category_include_project = config_get( 'user_count_category_include_project' );
	...
	$t_project_query = ( ON == $t_user_count_category_include_project ) ? 'b.project_id, ' : '';
	...
	while( $t_row = db_fetch_array( $t_result ) ) {
		...
		if( ( ON == $t_user_count_category_include_project ) && ( ALL_PROJECTS == $t_project_id ) ) {
	...
	foreach( $t_cache as $t_label => $t_item) {
		...
		user_count_helper_build_buglinks( $t_bug_link, $t_bugs_open, $t_bugs_resolved, $t_bugs_closed, $t_bugs_total );
		user_count_helper_print_row( string_display_line( $t_label ), $t_bugs_open, $t_bugs_resolved, $t_bugs_closed, $t_bugs_total, $t_bugs_ratio[0], $t_bugs_ratio[1] );
	}
}
 ...
 * @param array   $p_cache    user_count cache.
 * @return void
 */
function user_count_print_by_project($type=0,array $p_projects = array(), $p_level = 0, array $p_cache = null) {
	$t_project_id = helper_get_current_project();
	...
	if( null === $p_cache ) {
		...
		while( $t_row = db_fetch_array( $t_result ) ) {
			...
			user_count_helper_build_bugcount( $p_cache, $t_project_id, $t_status, $t_bugcount );			
		}
...
//
		...
		$sql = "select sum(work_hours) as total from {user_work_log}  where add_time>={$startTime} and add_time<={$endTime} and project_id=" . db_param() . " and user_id = " . db_param();
		$t_result = db_query($sql, [$t_project, $op_user_id]);
		$res = db_fetch_array($t_result);
		$p_ware_hours = $res['total'];
	   }else{
			$op_user_id = auth_get_current_user_id();
			$sql = "select sum(work_hours) as total from {user_work_log}  where project_id=" . db_param() . " and user_id = " . db_param();
			$t_result = db_query($sql, [$t_project, $op_user_id]);
			$res = db_fetch_array($t_result);
			$p_ware_hours = $res['total'];
	   }
...
#		user_count_helper_build_buglinks( $t_bug_link, $t_bugs_open, $t_bugs_resolved, $t_bugs_closed, $t_bugs_total );
		...
		user_count_helper_print_row( string_display_line( $t_name ), $t_bugs_open, $t_bugs_resolved, $t_bugs_closed, $t_bugs_total, $t_bugs_ratio[0], $t_bugs_ratio[1],$p_ware_hours);
		...
		if( count( project_hierarchy_get_subprojects( $t_project ) ) > 0 ) {
			...
			if( count( $t_subprojects ) > 0 ) {
				user_count_print_by_project( $t_subprojects, $p_level + 1, $p_cache );
			}
		}
	}
}
...
function user_count_print_by_project_hold($type=0,array $p_projects = array(), $p_level = 0, array $p_cache = null) {
	$t_project_id = helper_get_current_project();
 ...
 * @return void
 */
function user_count_print_developer_resolution( $p_resolution_enum_string ) {
	$t_project_id = helper_get_current_project();
	...
	foreach( $t_handler_res_arr as $t_handler_id => $t_arr2 ) {
		...
		if( $t_total > 0 ) {
			...
			$t_row_count++;
			echo '<td>';
			echo user_count_helper_get_developer_label( $t_handler_id );
			echo "</td>\n";
 ...
 * @return void
 */
function user_count_print_reporter_resolution( $p_resolution_enum_string ) {
	$t_reporter_user_count_limit = config_get( 'reporter_user_count_limit' );
	...
	foreach( $t_reporter_bugcount_arr as $t_reporter_id => $t_total_user_bugs ) {
		...
		if( $t_row_count >= $t_reporter_user_count_limit ) {
			break;
		}
 ...
 * @return void
 */
function user_count_print_reporter_effectiveness( $p_severity_enum_string, $p_resolution_enum_string ) {
	$t_reporter_user_count_limit = config_get( 'reporter_user_count_limit' );
 ...
 * @return array
 */
function user_count_helper_get_time_stats( $p_project_id ) {
