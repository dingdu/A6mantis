...
if($res) {
	$t_query = 'UPDATE {user_ext}
			SET skill=' . db_param() . ',user_type=' . db_param() . ',performance=' . db_param() . '
			WHERE user_id=' . db_param();
	$t_result = db_query($t_query, [$f_skill_ratio,$f_user_type,$f_performance,$f_user_id]);
}else {
	$t_query = 'INSERT INTO {user_ext}
					( user_id, skill, total_work_hours,user_type,performance)
				  VALUES
					( ' . db_param() . ', ' . db_param() . ', ' . db_param() . ', ' . db_param() .', ' . db_param() .')';
	db_query($t_query, [$f_user_id, $f_skill_ratio, 0,$f_user_type,$f_performance]);
}
